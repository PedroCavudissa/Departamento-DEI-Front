...
export class LancamentoComponent implements OnInit {
  ...
  tipos: TipoPauta[] = [
    ...
    { codigo: 3, descricao: 'Notas Do Exame Epoca Normal' },
    { codigo: 4, descricao: 'Notas Do Exame Epoca De Recurso' },
  ...
  ngOnInit(): void {
    ...
    this.lacamentoNotasService.getDisciplinasDoProfessor().subscribe({
      next: (dados) => {
        ...
        if (dados.length === 0) {
          this.erro = 'Nenhuma Disciplina Disponivel.';
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
        ...
        this.mensagem = '';
        this.erro = 'Extensao Invalida. So sao Aceites Ficheiros Excel (.xlsx ou .xls).';
        this.limparMensagensDepoisDeTempo();
      }
    }
  }
